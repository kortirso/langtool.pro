defmodule LangtoolProWeb.Api.V1.TasksControllerTest do
  ...
  describe "POST#detection" do
    test "returns code and name of locale", %{conn: conn} do
      ...
      conn = post conn, api_detection_path(conn, :detection), [file: %Plug.Upload{path: path, filename: "ru.yml"}, framework: "ruby_on_rails"]
    ...
    test "returns error message for invalid file", %{conn: conn} do
      ...
      conn = post conn, api_detection_path(conn, :detection), [file: %Plug.Upload{path: path, filename: "invalid.yml"}, framework: "ruby_on_rails"]
